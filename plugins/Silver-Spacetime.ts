...
class Plugin {
  ...
  massMove() {
    ...
    const asteroids = df.getMyPlanets()
        .filter(p => isAsteroid(p) && p.silver / p.silverCap >= 0.80 )
        .sort((a, b) => b.silver  > a.silver ? 0 : -1);
...
function distributeSilver(fromId, maxDistributeEnergyPercent) {
  ...
  if (!fromLoc) {
    console.log('location not exist for ', fromId);
    return;
  }
  const candidates_ = df.getPlanetsInRange(fromId, maxDistributeEnergyPercent)
      .filter(p => {
        if (!players.includes(p.owner) || p.planetLevel < 4 || p.planetType !== 3) {
          return false;
        }
        return true;
      })
