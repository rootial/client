...
let selectedSpacetimeLevels = [0, 9];
...
function LevelFilter({ levels, selectedLevels, onSelectLevel }) {
  ...
  return html`
    <div style=${buttonsRow} id="level-filter">
...
class Plugin {
  ...
  async render(container) {
    ...
    autoSecondsStepper.onchange = (evt) => {
      try {
        ...
        console.log("check", selectedSpacetimeLevels);
      } catch (e) {
    ...
    const spacetimeLevelFilter = html`<div style=${{ marginBottom: "3px" }}>
        Spacetime Planet Level Filter
      </div>
      <${LevelFilter}
        levels=${PLANET_LEVELS}
        selectedLevels=${selectedSpacetimeLevels}
        onSelectLevel=${(level) => {
          console.log("click level", selectedSpacetimeLevels);
          if (selectedSpacetimeLevels.length == 2) {
            selectedSpacetimeLevels = [level];
          } else {
            selectedSpacetimeLevels = [level, selectedSpacetimeLevels[0]];
          }
          console.log("new selected level", selectedSpacetimeLevels);
          console.log("container", this.container);
          console.log(document.getElementById("level-filter"));
        }}
      />`;


    this.root = render(html`${maxPendingLabel}`, container);
    ...
    // asteroid level filter

    // spacetime level filter
  ...
  massMove() {
    ...
    const asteroids = df
      .getMyPlanets()
      .filter((p) => isAsteroid(p) && p.silver / p.silverCap >= 0.8)
      .sort((a, b) =>
        b.silver / b.silverCap > a.silver / a.silverCap ? 0 : -1
      );
...
function distributeSilver(fromId, maxDistributeEnergyPercent) {
  ...
  while (energyBudget - energySpent > 0 && i < candidates_.length) {
    ...
    const energyNeeded = Math.ceil(
      df.getEnergyNeededForMove(fromId, candidate.locationId, energyLanding)
    );
